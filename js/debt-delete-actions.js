...
document.addEventListener('click', async (event) => {
  ...
  try {
    const { data: debt, error: debtError } = await supabase.from('money_debts').select('id,name,debt_type').eq('id', debtId).single();
    if (debtError) throw debtError;
    const { data: payments, error: paymentsError } = await supabase.from('money_payments').select('id,amount,reversed_at').eq('debt_id', debtId);
    ...
    if (allPayments.length) {
      const { error: deletePaymentsError } = await supabase.from('money_payments').delete().eq('debt_id', debtId);
