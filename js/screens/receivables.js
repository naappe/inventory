...
export function renderReceivables(bundle) {
  ...
  return `
    ...
    <section class="receivable-grid">${active.length ? active.map((r) => `<article class="panel receivable-card">
      <div class="debt-card-top"><div><span class="debt-type">Receivable</span><h2>${r.name}</h2></div><div class="row-end"><strong>${money(r.current_balance)}</strong><button class="text-button" data-action="edit-receivable" data-id="${r.id}">Edit</button><button class="text-button danger-text" data-action="delete-receivable" data-id="${r.id}" data-name="${r.name}">Delete</button></div></div>
