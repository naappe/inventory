...
export function renderReceivables(bundle) {
  ...
  return `
    ...
    <section class="receivable-grid">${active.length ? active.map((r) => `<article class="panel receivable-card">
      <div class="debt-card-top"><div><span class="debt-type">Receivable</span><h2>${r.name}</h2></div><strong>${money(r.current_balance)}</strong></div>
    ...
    ${settled.length ? `<article class="panel completed-debts"><div class="panel-head"><div><h2>Settled</h2><p>Money fully repaid to you.</p></div></div>${settled.map((r) => `<div class="simple-row"><div><b>${r.name}</b><span>Originally ${money(r.opening_balance)}</span></div><strong class="positive">Repaid</strong></div>`).join('')}</article>` : ''}
