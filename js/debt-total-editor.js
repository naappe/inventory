...
async function getDebtEditorModel(id) {
  ...
  const { data: payments, error: paymentError } = await supabase
    .from('money_payments')
    .select('id,month_id,amount,payment_date,reversed_at,created_at')
  ...
  const paidOtherMonths = active
    .filter((p) => p.month_id !== month.id)
  ...
  return {
    ...
    month,
    monthKey,
    currentMonthPaid: Number(currentMonthPaid.toFixed(2)),
    paidOtherMonths: Number(paidOtherMonths.toFixed(2)),
    paymentDate,
  };
}
...
function openMonthlyDebtEditor({ debt, month, monthKey, currentMonthPaid, paidOtherMonths, paymentDate }) {
  ...
  const preview = (form) => {
    try {
      ...
      const next = debtMonthEditPreview({
        totalAmount: opening,
        paidBeforeMonth: paidOtherMonths,
        currentMonthPaid,
        newMonthPayment: monthPayment,
      });
      setSheetPreview(`
        <div class="payment-preview">
          <span>Paid in other months</span><strong>${money(next.paidBeforeMonth)}</strong>
          <span>This month payment</span><strong>${money(next.newMonthPayment)}</strong>
          <span>Remaining balance after save</span><strong>${money(next.remainingBalance)}</strong>
        </div>`);
    } catch (error) {
      setSheetPreview(`<div class="warning-box">${error.message}</div>`);
    }
  };
  ...
  openSheet({
    ...
    body: `${field.text('name', debt.debt_type === 'credit' ? 'Credit name' : 'Loan name', debt.name, 'required')}
      ...
      <div class="helper">Paid in other months: <strong>${money(paidOtherMonths)}</strong>. Those records stay unchanged.</div>
      ${field.money('monthPayment', `Payment for ${document.getElementById('month-title')?.textContent?.trim() || monthKey}`, currentMonthPaid ? currentMonthPaid.toFixed(2) : '', 'min="0"')}
      ${field.date('paymentDate', 'Payment date', paymentDate, 'required')}
      <div class="helper">This amount belongs only to the selected month. Next month can be a completely different amount.</div>
    ...
    onReady: (form) => {
      preview(form);
      form.querySelector('[name="openingBalance"]')?.addEventListener('input', () => preview(form));
      form.querySelector('[name="monthPayment"]')?.addEventListener('input', () => preview(form));
    },
    onSubmit: async (values) => {
      ...
      const model = debtMonthEditPreview({
        totalAmount: opening,
        paidBeforeMonth: paidOtherMonths,
        currentMonthPaid,
        newMonthPayment: requestedMonthPayment,
      });
      const plan = Math.max(0, Number(values.monthlyPlan || 0));
      const apr = values.apr === '' ? null : Math.max(0, Number(values.apr || 0));

      const { data, error } = await supabase.rpc('money_edit_debt_month', {
        p_debt_id: debt.id,
        p_month_id: month.id,
        p_name: String(values.name || '').trim(),
        p_debt_type: values.type,
        p_total_amount: opening,
        p_monthly_plan: plan,
        p_apr: apr,
        p_month_payment: model.newMonthPayment,
