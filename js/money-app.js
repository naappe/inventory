...
function openDeleteItemSheet(source){const itemId=source.dataset.id;const monthItemId=source.dataset.monthItemId;const name=source.dataset.name||'this expense';openSheet({title:'Delete expense?',subtitle:name,body:`<div class="confirm-copy">This removes the expense from the current/future plan when safe. If it already has payment history, that history and its month snapshot stay preserved.</div>`,submitLabel:'Delete expense',onSubmit:async()=>{const result=await api.archiveItemForMonth(itemId,monthItemId);await reload(result.historyPreserved?'Expense stopped for future months. Paid history was preserved.':'Expense removed from the plan.');}});}
function openReverseSheet(payment){openSheet({title:'Reverse payment?',subtitle:`${money(payment.amount)} will be marked reversed. The original record stays in history.`,body:`${field.text('reason','Reason','Correction','required')}<div class="warning-box">Debt balances are restored automatically when relevant.</div>`,submitLabel:'Reverse payment',onSubmit:async(v)=>{await api.reversePayment(payment.id,v.reason);await reload('Payment reversed');}});}
function openReverseReceivableSheet(tx){openSheet({title:'Reverse Money Lent transaction?',subtitle:`${money(tx.amount)} · ${tx.transaction_type}`,body:`${field.text('reason','Reason','Correction','required')}<div class="warning-box">The receivable balance will be restored to the correct amount.</div>`,submitLabel:'Reverse transaction',onSubmit:async(v)=>{await api.reverseReceivableTransaction(tx.id,v.reason);await reload('Receivable transaction reversed');}});}
